...
<<<<<<< HEAD
df_agg_diff[numeric_cols] = df_agg_diff[numeric_cols].apply(lambda col:
 (col.astype(float) - float(median_agg.get(col.name, 1.0))) /
  max(float(median_agg.get(col.name, 1.0)), 0.0001)).fillna(0).replace([float('inf'), -float('inf')], 0)
=======
...
>>>>>>> fee36c11421664cd37a8cd71009bc73d2930877f
# build dashboard
